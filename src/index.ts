...
export class ShamirsSecretSharing {
  /* -------------------------------------------------------------------------- */
  /*                                  Variables                                 */
  /* -------------------------------------------------------------------------- */
   ...
   */
  public static splitString(secret: string, totalShards: number, threshold: number): string[] {
   ...
   * @returns {string[]} An array of shards.
   */
  public static splitBuffer(secretBuffer: Buffer, totalShards: number, threshold: number): string[] {
    ...
    const padding = (this.FOUR_BYTES - (buffer.byteLength % this.FOUR_BYTES)).toString();

    // Initialize the shard table
    for (let i = 0; i < totalShards; i++) {
      fragShardTable[i] = [i.toString(this.HEX_BASE).padStart(4, '0'), thresholdHex, padding];
    }
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      for (let j = this.FOUR_BYTES - 1; j >= 0; j--) {
        fragment = fragment * this.BYTE_LENGTH + (buffer[i * this.FOUR_BYTES + j] || 0);
      }
  ...
  public static combineBuffer(shards: string[]): Buffer {
    ...
    return buffer.slice(0, buffer.byteLength - (Math.round(remainder / shards.length) % this.FOUR_BYTES));
  }
   ...
   * @returns {string[]} An array of shards.
   */
  private static splitFrag = (secret: number, n: number, k: number): string[] => {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      for (let j = 1; j < k; j++) {
        accum = (accum + ((coeff[j] * (BigInt(i ** j) % this.stdPrime)) % this.stdPrime)) % this.stdPrime;
      }
   ...
   */
  private static combineFrag = (shards: { id: number; data: bigint }[]): number => {
    let accum: bigint = 0n,
      start: number,
      next: number;
    ...
    for (let i = 0; i < shards.length; i++) {
      ...
      accum = (this.stdPrime + accum + shards[i].data * numerator * this.modInv(denominator)) % this.stdPrime;
      while (accum < 0n) {
        accum += this.stdPrime;
      }
    }
  ...
  private static modInv = (k: bigint): bigint => {
    k = k % this.stdPrime;
    let r = k < 0 ? -this.gcd(this.stdPrime, -k)[2] : this.gcd(this.stdPrime, k)[2];
    return (this.stdPrime + r) % this.stdPrime;
  };
}

export const SSS = ShamirsSecretSharing;
